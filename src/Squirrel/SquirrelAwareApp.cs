...
namespace Squirrel
{
    ...
    public static class SquirrelAwareApp
    {
        ...
        /// arguments. In your app, leave this as null.</param>
        public static void HandleEvents(
            Action<string> onInitialInstall = null,
            Action<string> onAppUpdate = null,
            Action<string> onAppObsoleted = null,
            Action<string> onAppUninstall = null,
            Action onFirstRun = null,
            string[] arguments = null)
        {
            Action<string> defaultBlock = (v => { });
            var args = arguments ?? Environment.GetCommandLineArgs().Skip(1).ToArray();
            if (args.Length == 0) return;
